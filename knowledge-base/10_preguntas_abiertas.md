...
| PA-22 | Las claves de Plus son strings libres configurables por tenant (no enum global). El mapeo materia→clave se define como campo `clave_plus` en `Materia`. El ADMIN del tenant gestiona la grilla desde F10.4. Materias sin clave no generan Plus. | [05_reglas_de_negocio.md RN-33](05_reglas_de_negocio.md) |
| PA-23 | Plus acumula N veces si el docente tiene N comisiones de la misma clave (RN-33/RN-34). Sin tope de acumulación. Mismo criterio para todos los roles (TUTOR, PROFESOR, COORDINADOR). NEXO tiene tratamiento contable separado por flag `es_nexo` en `Liquidacion`, no por lógica de Plus distinta. | [05_reglas_de_negocio.md RN-33, RN-34](05_reglas_de_negocio.md) |
