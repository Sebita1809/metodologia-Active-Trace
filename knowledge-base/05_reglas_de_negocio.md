...
Los plus son adicionales identificados por una **clave de categoría** cruzada con el rol del docente. Las claves son strings libres configurables por tenant (no enum global); el ADMIN las define en la grilla salarial (F10.4). El mapeo materia→clave se registra como campo `clave_plus` en `Materia`; materias sin clave no generan Plus. Si un docente tiene asignaciones en N comisiones de la misma categoría, **acumula N veces** el plus correspondiente. No existe tope de acumulación. La misma lógica aplica a todos los roles (TUTOR, PROFESOR, COORDINADOR); el rol NEXO no usa esta lógica sino el flag `es_nexo` en `Liquidacion`.

### RN-34 — Cálculo de liquidación mensual
La liquidación mensual de un docente se calcula como:
```
Total = Base(rol vigente al mes) + Σ(Plus(clave_materia, rol) × N_comisiones_activas_de_esa_clave)
```
donde `N_comisiones_activas_de_esa_clave` es la cantidad de comisiones activas del docente cuyas materias tienen esa clave de categoría en el período liquidado. Materias sin `clave_plus` no aportan Plus al total.
